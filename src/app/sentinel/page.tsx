                ...
                <div key={log.id} className="p-4 flex items-start gap-4 hover:bg-primary/5 transition-colors border-l-2 border-transparent hover:border-primary group">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex-grow">
                      <p className={`font-mono text-sm ${log.type === 'NORMAL' ? 'text-green-400' : log.type === 'SYSTEM_LOCKDOWN' ? 'text-red-400 font-bold' : log.type === 'RATE_LIMIT_EXCEEDED' ? 'text-orange-400' : 'text-primary'}`}>
                        {log.message}
                      </p>
                      <p className="text-xs text-white/40 mt-1 uppercase tracking-wider font-semibold">{log.type.replace('_', ' ')}</p>
